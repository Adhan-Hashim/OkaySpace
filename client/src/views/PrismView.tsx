      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', width: '100%', flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--sp-8)', borderBottom: '2px solid #000000', paddingBottom: 'var(--sp-4)' }}>
          <div className="view-header-left">
            <h1 className="vd-title-large">PRISM</h1>
            <p className="vd-subtitle">Refract a difficult thought through six healing perspectives</p>
          </div>
          {prismThought && (
            <button className="vd-btn-pill-secondary" onClick={clearPrism} id="prism-clear" style={{ minHeight: '40px', height: '40px', padding: '0 1.25rem', fontSize: '0.85rem' }}>
              ↻ &nbsp;New Thought
            </button>
          )}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {!prismThought && !prismLoading && (
            <motion.div
              ...
              style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-8)', padding: 'var(--sp-6) 0' }}
            >
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--sp-4)', textAlign: 'center' }}>
                ...
                <div>
                  ...
                  <p style={{ color: '#666666', maxWidth: '500px', lineHeight: 1.7, fontSize: '1rem', margin: '0 auto' }}>
              ...
              <div className="vd-card-flat" style={{ maxWidth: '600px', width: '100%', margin: '0 auto' }}>
                <div style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 800, fontSize: '0.85rem', textTransform: 'uppercase', color: '#000000', marginBottom: 'var(--sp-2)' }}>Your thought</div>
                <textarea
                  ...
                  rows={4}
                  id="prism-input"
                  style={{ fontSize: '1.05rem', lineHeight: 1.65, border: '1.5px solid #000000', borderRadius: '8px' }}
                />
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'var(--sp-4)' }}>
                  <span style={{ fontSize: '0.75rem', color: '#666666', fontWeight: 500 }}>Enter to refract · Shift+Enter for new line</span>
                  <button
                    className="vd-btn-pill"
                    onClick={submitThought}
                    disabled={!input.trim()}
                    id="prism-submit"
                    style={{ minHeight: '40px', height: '40px', padding: '0 1.5rem', fontSize: '0.85rem' }}
                  >
                    Refract
                  </button>
                </div>
              ...
              {/* How it works mini */}
              <div style={{ display: 'flex', gap: 'var(--sp-3)', flexWrap: 'wrap', justifyContent: 'center', maxWidth: '600px', margin: '0 auto' }}>
                {FACET_TYPES.map((f) => (
                  <div key={f.key} style={{
                    display: 'flex', alignItems: 'center', gap: 'var(--sp-2)',
                    padding: 'var(--sp-2) var(--sp-4)',
                    background: '#f3f4f6', borderRadius: 'var(--r-full)',
          ...
          {prismLoading && (
            ...
            >
              ...
              <div>
                <h3 style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 800, fontSize: '1.5rem', color: '#000000', marginBottom: 'var(--sp-2)' }}>
              ...
              <div style={{ display: 'flex', gap: 'var(--sp-2)', justifyContent: 'center' }}>
                {[0, 0.2, 0.4].map((d, i) => (
                  <motion.div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: '#000000' }}
          ...
          {prismThought && !prismLoading && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.4 }}
              style={{ flex: 1, paddingBottom: 'var(--sp-10)' }}
            >
              {/* Original thought banner */}
              <div className="vd-card-flat" style={{ marginBottom: 'var(--sp-8)' }}>
                <div style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 800, fontSize: '0.85rem', textTransform: 'uppercase', color: '#666666', marginBottom: 'var(--sp-2)' }}>Your original thought</div>
                <div style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.4rem', fontStyle: 'italic', fontWeight: 600, color: '#000000' }}>
                  "{prismThought}"
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {FACET_TYPES.map((facetType, i) => {
                  ...
                  return (
                    <div
                      key={facetType.key}
                      className="vd-accordion-item"
                      onClick={() => setExpandedFacet(isExpanded ? null : facetType.key)}
                      id={`facet-${facetType.key}`}
                    >
                      <div className="vd-accordion-header">
                        <span className="vd-accordion-title">{facetType.label}</span>
                        <span className="vd-accordion-icon">{isExpanded ? '−' : '+'}</span>
                      </div>
                      <AnimatePresence initial={false}>
                        {isExpanded && (
                          <motion.div
                            ...
                            className="vd-accordion-content"
              ...
              {/* Action footer */}
              <div style={{ display: 'flex', gap: 'var(--sp-4)', marginTop: 'var(--sp-12)', justifyContent: 'center' }}>
                <button className="vd-btn-pill" onClick={clearPrism}>Try Another Thought</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
